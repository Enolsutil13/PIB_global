...
def show_landing(): # función para gestionar la landing 
    ...
    st.markdown('<p class="big-font">At the European Central Bank, we have the best analysts for data operations. Our team of Upgraders will present the main results on their Global Economic Indicators research.</p>', unsafe_allow_html=True)
...
def analisis(): #función para mostrar un conjunto de gráficas en formato matriz 
    ...
    with col1:
        ...
        fig = px.line(df2, x="Year", y="GDP_growth", color="Continent", 
                    title="GDP growth rate by continent, in % of previous year, in the period 1970,2021",
                    labels={"GDP_growth": "GDP growth rate, in % of previous year", "Year": "Year", "Continent": "Continent"},
...
def power_bi():                                  
    ...
    # Mostramos el iframe del dashboard de Power BI
    st.components.v1.iframe(powerbi_url, height=600)

# Definimos la función predictor para añadir las predicciones de azure y series de tiempo
def predictor():   
    ...
    # Realizamos la inferencia al enviar el formulario
    if submit_button:
        # Convertimos el año en la fecha esperada por el modelo
        ...
        if response.status_code == 200:
            ...
            st.write("Response JSON:", result)  # Imprimimos la estructura de la respuesta para su depuración

            # Conviene ajustar esta parte según la estructura real de la respuesta
            ...
            if isinstance(result, list) and len(result) > 0:
                ...
                prediction = result[0]  # Ajustamos esto según sea necesario
                # Almacenamos el resultado de la predicción
    ...
    # Mostramos la gráfica actualizada con las predicciones 
    if st.session_state.predictions:
        df = pd.DataFrame(st.session_state.predictions)
        if 'GDP' in df.columns:
            st.line_chart(df.set_index('Year')['GDP'])
        else:
            st.error("Column 'GDP' not found in the data.")
    ...
    with st.expander("Model comparison"):
        ...
        # DIbujamos intervalos de confianza
        #plt.fill_between(dfnp['ds'], dfnp['Lower'], dfnp['Upper'], color='red', alpha=0.1)
